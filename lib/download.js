...
async function downloadFromPostman(Config, {dryRun = false} = {}) {
  ...
  function downloadCollection() {
    return Promise.try(async () => {
      ...
      .then(async (jsonRes) => {
        ...
        await writeJsonIndentation(postmanIndentedSpecPath, jsonRes);
        return postmanSpecPath;
      })
      .then((jsonRes) => {
        if (dryRun) {
          console.log('dry run. Response was');
  ...
  return Promise.try(async () => {
    return Promise.all([downloadCollection(), downloadEnvironment()]);
  })
