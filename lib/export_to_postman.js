#!/usr/bin/env node
async function exportToPostman(Config, {dryRun = false}) {
  ...
  return Promise.try(async () => {
    ...
    .delay(1000)
    .then(async function() {
      originalSpec = require(postmanSpecPath);
      ...
      await maybeSaveOrPrint(postmanSpecPath, _.cloneDeep(originalSpec));
      ...
      await maybeSaveOrPrint(swaggerIndentedSpec, convertedSpec);
      ...
      let pmMethodFile = postmanRoutesPath.replace(
        '.json',
        '_methodpaths.json'
      );
      ...
      let swMethodFile = swaggerRoutesPath.replace(
        '.json',
        '_methodpaths.json'
      );
