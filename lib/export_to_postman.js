...
var originalSpec = require(postmanSpecPath),
  postmanPaths = {},
  swaggerPaths = {},
  fileArray = [];
debug(`reading from ${chalk.bold.green(path.basename(postmanSpecPath))}`);
...
function fixUrlParams(url) {
  ...
  if (url.path[offSet + 2] === ':id') {
    ...
    let singular = entityUtils.relationToSingular(entityName);
  ...
  url.query = _.compact(
    _.map(url.query, query => {
      if (query.key === '') {
        ...
        query.description = 'Opcional, filtro en forma de String JSON encoded';
...
function mergeResponseBodies(value, postmanItem, debugFN, method_path) {
  ...
  let codesOnlyInPostman = _.difference(
    ...
    codesOnlyInSwagger = _.difference(swaggerExampleCodes, postmanExampleCodes),
...
Promise.resolve()
  .then(async function() {
    let swaggerContentRaw = require(swaggerSpecPath),
      originalInfo = _.cloneDeep(originalSpec.info),
    ...
    await writeJsonIndentation(postmanSpecPath, _.cloneDeep(originalSpec));
    ...
    var conversionResult = Converter.convert(swaggerContentRaw),
      pmCollection = {
        collection: {
          info: originalInfo
        }
    ...
    let convertedSpecRaw = Object.assign(
        {info: originalInfo},
        _.omit(conversionResult.collection, ['info'])
      ),
      {parsedCollection: convertedSpec} = normalizeCollectionIds(
        convertedSpecRaw
      );

    convertedSpec = convertedSpec.toJSON();
    await writeJsonIndentation(swaggerIndentedSpec, convertedSpec);

    let {
      parsedCollection: normalizedSpec,
      methodPaths: postmanMethodPaths,
      ancestor: postmanAncestor
    } = normalizeCollectionIds(originalSpec);

    await writeJsonIndentation(postmanIndentedSpecPath, normalizedSpec);
    ...
    await writeJsonIndentation(
      postmanRoutesPath.replace('.json', '_methodpaths.json'),
      postmanMethodPaths
    );
    ...
    unwrapItem(convertedSpec, []);

    Object.assign(pmCollection.collection, convertedSpec);
    ...
    let info = {
      ...
      name: originalSpec.info.name || Config.POSTMAN_COLLECTION_NAME,
    ...
    // info.description.content = originalSpec.info.description;

    pmCollection.collection.info = info;

    // Guarda los endpoints presentes en postman
    await writeJsonIndentation(postmanRoutesPath, postmanPaths);
    ...
    // Combina la colección de Postman existente con la que se generó de Swagger
    //unwrapItem(convertedSpec, []);
    await writeJsonIndentation(swaggerRoutesPath, swaggerPaths);
    await writeJsonIndentation(
      swaggerRoutesPath.replace('.json', '_methodpaths.json'),
      swaggerMethodPaths
    );
    ...
    await writeJsonIndentation(postmanNormalizedPath, {
    ...
    await writeJsonIndentation(swaggerAncestorPath, swaggerAncestor);
    await writeJsonIndentation(
      postmanNormalizedPath.replace('.json', '_2.json'),
      {
        collection: finalCollection2
      }
    );
    return;
  })
