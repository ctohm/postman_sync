...
const tslib_1 = require('tslib'),
  ...
  commandOptions_1 = tslib_1.__importStar(
    require('dtsgenerator/dist/commandOptions')
  ),
      ...
      switch (_g.label) {
        case 0:
          ...
          if (config_1.default.input.stdin) {
            ps.push(
              core_1.readSchemaFromStdin().then(function (s) {
                contents.push(s);
              })
            );
          }
          try {
            for (
              _a = tslib_1.__values(config_1.default.input.files),
                _b = _a.next();
              !_b.done;
              _b = _a.next()
            ) {
              pattern = _b.value;
              ps.push(
                core_1.readSchemasFromFile(pattern).then(function (ss) {
          ...
          try {
            for (
              _c = tslib_1.__values(config_1.default.input.urls),
                _d = _c.next();
              !_d.done;
              _d = _c.next()
            ) {
              url = _d.value;
              ps.push(
                core_1.readSchemaFromUrl(url).then(function (s) {
                  contents.push(s);
                })
              );
            }
      ...
      switch (_a.label) {
        case 0:
          let args = process.argv
            .slice(0, 2)
            .concat(['-o', `${outputFile}`, `${openApiSrc}`]);
          console.log(args);
        ...
        case 4:
          ...
          if (commandOptions_1.default.out) {
            mkdirp_1.default.sync(
              path_1.default.dirname(commandOptions_1.default.out)
            );
            fs_1.default.writeFileSync(commandOptions_1.default.out, result, {
              encoding: 'utf-8',
            });
          } else {
            console.log(result);
