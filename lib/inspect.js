...
function countEvents(
  ...
  if (events && events.length) {
    ...
    mkdirp.sync(`${process.env.PROJECT_ROOT}/${saveFolder}`);

    accum = accum.concat(
      events.map(event => {
        let {exec, ...cleanEvent} = event,
          fileName = `${process.env.PROJECT_ROOT}/${saveFolder}/${_.snakeCase(
            cleanEvent.name.split('/').pop()
          )}.js`,
          dirname = `${process.env.PROJECT_ROOT}/postman_testscripts/`;
  ...
  if (shouldRecurse) {
    ...
    accum = accum.concat(
      items.reduce((accum2, subGroup, index) => {
        ...
        mkdirp.sync(`${process.env.PROJECT_ROOT}/${newSaveFolder}`);
