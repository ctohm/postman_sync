...
function getOpenApiAndJsonSchemaPaths({
  ...
  BASE_FOLDER = path.resolve(`${__dirname}/../../`),
  SPECS_FOLDER,
  COLLECTIONS_FOLDER,
  ENVIRONMENTS_FOLDER
}) {
  SPECS_FOLDER = SPECS_FOLDER || path.resolve(`${BASE_FOLDER}/specs`);
  COLLECTIONS_FOLDER = path.resolve(`${BASE_FOLDER}/json_collections`);
  ENVIRONMENTS_FOLDER =
    ENVIRONMENTS_FOLDER || path.resolve(`${BASE_FOLDER}/json_environments`);
