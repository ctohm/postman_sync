...
const Promise = require('bluebird'),
  ...
  chalk = require('chalk');
function getCollectionScripts(collection) {
  ...
  const clientePrescript = fs.readFileSync(
      `${process.env.PROJECT_ROOT}/postman_testscripts/01_cliente/01_cliente_prerequest.js`,
      'utf8'
    ),
    preScript = fs.readFileSync(
      `${process.env.PROJECT_ROOT}/postman_testscripts/root_prerequest.js`,
      'utf8'
    ),
    testScript = fs.readFileSync(
      `${process.env.PROJECT_ROOT}/postman_testscripts/root_test.js`,
      'utf8'
    );
